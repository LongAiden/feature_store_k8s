...
# This secret is used by MinIO itself, by Spark, and by the Hive Metastore.
...
# NOTE ON HIVE METASTORE S3 SECRET:
# The secret 'hive-metastore-s3-secret' is no longer created because the Hive Metastore
# is now configured in values.yaml to use the 'minio-credentials' secret for S3 access.
# This ensures Spark and Hive Metastore use the same credentials.
